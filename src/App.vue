<script setup>
  ...
  const charGender = ref(false);
  const charMoney = ref(0);

  let fastSlots = [1, 2, 3, 4, 5],
      clickTime = 0,
      invOpacity = 1,
      invOldOpacity = -1,
      ItemStack = -1,
  ...
  const onBlurStack = () => {
      if (StackValue.value < 1) StackValue.value = 1;
      else if (StackValue.value > selectItem.Count - 1) StackValue.value = selectItem.Count - 1;
  }
  ...
  const onStack = () => {
      if (selectItem.use === stageItem.useItem) {
          ...
          if (ItemStack == 2) {
              ...
              if (MaxStakcItems > 0) StackValue.value = MaxStakcItems;
          }
          executeClient ("client.gamemenu.inventory.stack", selectArrayName, selectIndex, ItemStack, StackValue.value);
          itemNoUse (17);
      }
  }
  ...
  const onBuy = () => {
      if (selectItem.use === stageItem.useItem) {
          const selectIndex = selectItem.index;
          const selectArrayName = selectItem.arrayName;
          executeClient ("client.gamemenu.inventory.buy", selectArrayName, selectIndex, StackValue.value);
          itemNoUse (17);
      }
  }
  ...
  const rangeslidercreate = (max) => {
      StackValue.value = Math.round(max / 2);
      setTimeout(() => {
  ...
  let boxItemInfo = ref(null);
  let boxInfoLeft = 0;
  let boxInfoTop = 0;

  if (boxItemInfo.value) {
      boxInfoLeft = fixOutToCenter($coords.x, boxItemInfo.value);
      boxInfoTop = fixOutToTop($coords.y, boxItemInfo.value);
  }

  let boxPopup = ref(null);
...
<template>
  <div class="box-inventory">
      <div v-if="selectItem.use === stageItem.move"
        class="dragonDrop"
  ...
  <div v-if="infoItem !== defaulHoverItem && selectItem.use !== stageItem.move" class="box-item-info" :style="{top: `${boxInfoTop}px`, left: `${boxInfoLeft}px`}" ref="boxItemInfo">
      <div class="bg-info">
          <div class="title">{{getName(infoItem, infoItem.arrayName)}}</div>
  ...
  <div v-else-if="selectItem.use === stageItem.useItem && ItemStack === -1" ref="boxPopup" class="box-use" 
    ...
    <div class="box-item-weapon">
        <div class="bg-info">
            <div class="box-column">
                <div class="box-item-weapon__image" :style="{backgroundImage: `url(${getPng(selectItem, itemsInfo[selectItem.ItemId])})`}"></div>
                <div class="box-item-weapon__line"></div>
                <div class="title">
                    {{getName(selectItem, selectItem.arrayName)}}
                </div>
            ...
            <template v-if="itemsInfo[selectItem.ItemId].functionType === ItemType.Weapons && inventoryWeapons[selectItem.ItemId]">
              <div class="title left">{{translateText('player1', 'Характеристики')}}</div>
                ...
                <div v-if="inventoryWeapons[selectItem.ItemId].accuracy" class="box-item-weapon__element">
                ...
                <div v-if="inventoryWeapons[selectItem.ItemId].ammo" class="box-item-weapon__element">
                    <div>{{translateText('player1', 'Патронов в магазине')}}</div>
                    <div>{{inventoryWeapons[selectItem.ItemId].ammo}}</div>
                </div>
            </template>
        </div>
        <div v-if="itemsInfo[selectItem.ItemId].functionType === ItemType.Weapons" class="box-item-weapon__serial">
        ...
        <div v-else-if="OtherInfo.Id == otherType.Tent && selectItem.Price" class="box-item-weapon__serial">
    ...
    <template v-if="OtherInfo.Id === otherType.Tent && OtherInfo.IsMyTent && selectItem.arrayName === 'other'">
      <div class="item" @click="onTransfer">
        <span class="inventoryicons-backpack inventoryicon-use"></span>
        {{translateText('player1', 'забрать')}}
      </div>
    </template>
    <template v-else>
      <div v-if="getItemsUse(selectItem) !== false" class="item" @click="onUseItem">{{getItemsClickInfo(selectItem)}}</div>
      <div v-if="getToPut(selectItem.ItemId) !== false && getDropItem(selectItem.arrayName, selectItem.ItemId) !== false && selectItem.arrayName === 'inventory'" class="item" @click="onToPut">
      ...
      <div v-else-if="OtherInfo.Id === otherType.Tent && OtherInfo.IsMyTent && (selectItem.arrayName === 'inventory' || selectItem.arrayName === 'backpack' )" class="item" @click="onTransfer">
      ...
      <div v-if="getDropItem(selectItem.arrayName, selectItem.ItemId) !== false" class="item" @click="onDropItem">
        <span class="inventoryicons-drop inventoryicon-use"></span>
        {{translateText('player1', 'выбросить')}}
      </div>
    </template>
  </div>
              ...
              <div v-if="maxSlotBackpack > 0 && ItemsData['backpack'].length && ItemsData['backpack'][0] !== undefined && ItemsData['backpack'][0].use !== undefined" :style="{opacity: `${invOpacity}`}">
                  <div class="box-column">
                      ...
                      <div class="inventory__text">
                          <span class="inventoryicons-backpack inventory__icon"></span>
                ...
                <div v-if="OtherInfo.Id > otherType.None" class="box-other" @mouseenter="e => mainInventoryArea = true" @mouseleave="e => mainInventoryArea = false">
                    <div class="box-between height-box">
                        <div class="box-column height-box">
                            ...
                            <div class="inventory__text">
                              <span :class="`${otherName[OtherInfo.Id].icon} inventory__icon`"></span>
                              {{otherName[OtherInfo.Id].descr}}
                            </div>
                        </div>
                    </div>
                    <div class="box-list">
            ...
            <div class="box-width-356 trade">
                <div class="box-between" @mouseenter="e => mainInventoryArea = true" @mouseleave="e => mainInventoryArea = false">
                    <div class="box-column">
                        ...
                        <div class="inventory__text">
                          <span :class="`trade-square ${tradeInfo.YourStatus ? 'active' : ''}`">
                            {{!tradeInfo.YourStatus ? "&#10006;" : "&#10003;"}}
                          </span>
                          {{!tradeInfo.YourStatus ? translateText('player1', 'Не готов к обмену') : translateText('player1', 'Готов к обмену')}}
                        </div>
                    </div>
